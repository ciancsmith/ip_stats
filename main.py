...
var1 = IntVar()
var2 = IntVar()
c1 = Checkbutton(master, text="get_ip_stats", variable=var1).grid(row = 0, column= 2, padx=2, pady=2, sticky=W)
c2 = Checkbutton(master, text="get_subnet_stats", variable=var2).grid(row = 1, column = 2, padx=2, pady=2, sticky=W)

# infinite loop which can be terminated by keyboard 
# or mouse interrupt 
action = Button(master, text="Process", command=stats, width=10).grid(row=2, column=1, sticky=W)
quit = Button(master, text="Quit", command=master.quit, width=10).grid(row=2, column=2, sticky=W)
mainloop() 